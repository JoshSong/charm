...
int debug_mode = DEBUG_TOSCREEN;//DEBUG_NONE;
...
bool setupHistogram(HistogramInfo &histInfo, const char *histoFile, int mode)
{
	...
	else if (mode != MODE_TRAINCLASSIFY)
	{
...
bool setupCamCalib(cv::Mat &cameraMatrix, cv::Mat &distCoeffs)
{
...
void *webcamCapture(void *cam)
{
	...
	while(running) { webcam->read(camImage); cv::imshow("vid", camImage); cv::waitKey(15); }
...
int main(int argc, char *argv[])
{
	...
	//TODO: set webcam properties to get best results!!!

	cv::Mat cameraMatrix, distCoeffs;
	if (!setupCamCalib(cameraMatrix, distCoeffs) || mode == MODE_CALIBRATE) { // if there is no camera calib file, or if we are explicitly in calib mode then do calib and quit.
		cameraCalibrationRoutine(webcam);
		return EXIT_SUCCESS;
	}
	...
	while (1)
	{
		...
		if (FindFrameAndCoins(img, cameraMatrix, distCoeffs, img_output, contours, possible_coins, table_rotation, evp))
		{
			...
			if (!GetCoinClassifyErrors(img, possible_coins, histInfo, 10.f, mode == MODE_TRAINCLASSIFY))
			...
			float confidence_threshold = 0.0001f;
			...
			for (int i = 0; i < possible_coins.size(); i++)
			{
				...
				for (int j = 0; j < num_persistant_coins; j++)
				{
					...
					if (dist.x*dist.x + dist.y*dist.y < 100.f) // if dist < 10mm then assume this is the same coin
					{
						...
						cv::Scalar col = (persistant_coins[j].confidence > confidence_threshold) ? cv::Scalar(0,255,0) : cv::Scalar(0,200,200);
			...
			if (frame_count > 0)
			{
				float dt = (float)((camTime - lastCamTime)/cv::getTickFrequency());
				float angle_diff = table_rotation - lastTableAngle;
				if (angle_diff < -0.1f) angle_diff += 2*PI; // TODO: hmmmmm...............
				tableSpeed = angle_diff/dt;
				avgTableSpeed = (avgTableSpeed*frame_count + tableSpeed) / (frame_count+1.f);
			}
			lastCamTime = camTime;
			...
			{
				if (persistant_coins[i].confidence > confidence_threshold)
				{
					total_coin_value += COIN_VALUES[persistant_coins[i].type];
					type_count[persistant_coins[i].type]++;
				}
				sprintf(coin_type_text, "Coin %d: %s (conf: %f, count: %d, dia: %.2f, loc:[%.1f, %.1f, %.1f])", i, COIN_NAMES[persistant_coins[i].type], persistant_coins[i].confidence, persistant_coins[i].count, persistant_coins[i].diameter, persistant_coins[i].location.x, persistant_coins[i].location.y, persistant_coins[i].location.z); 
			...
			sprintf(buf, "Table Speed: %.4f (avg: %.4f)", tableSpeed, avgTableSpeed);
			cv::putText(window, buf, cv::Point(680, 430), cv::FONT_HERSHEY_TRIPLEX, 0.4f, cv::Scalar(0,0,0));

			/*cv::Mat r_vec(3,1,CV_64F);
			if (frame_found) cv::Rodrigues(cam_R, r_vec);
			sprintf(buf, "Camera Position: (%.2f, %.2f, %.2f)", cam_t.at<double>(0),cam_t.at<double>(1),cam_t.at<double>(2));
			cv::putText(window, buf, cv::Point(680, 400), cv::FONT_HERSHEY_TRIPLEX, 0.4f, cv::Scalar(0,0,0));
			sprintf(buf, "Camera Orientation: (%.2f, %.2f, %.2f)", r_vec.at<double>(0),r_vec.at<double>(1),r_vec.at<double>(2));
			cv::putText(window, buf, cv::Point(680, 420), cv::FONT_HERSHEY_TRIPLEX, 0.4f, cv::Scalar(0,0,0));*/
