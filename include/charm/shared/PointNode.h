...
public:
	...
	const PointNode& operator = (const PointNode& n); // = operator overload

	// Constructor
	PointNode(Point p, ros::Time time, int index = -1, int parent_index = -1, float path_cost = 0, float heuristic_cost = 0);
	PointNode(float x, float y, ros::Time time, int index = -1, int parent_index = -1, float path_cost = 0, float heuristic_cost = 0);
