...
void MainPlanner::go() {
	...
	PLAN_TIME_OFFSET = 50;
	int plan_time_per_path = 3 * items.size();
	...
	PointNode start_node(pos, ros::Time::now() + ros::Duration(PLAN_TIME_OFFSET * 0.001));

	ros::Time plan_start_time = ros::Time::now();
	ros::Time plan_end_time = plan_start_time + ros::Duration(PLAN_TIME_OFFSET * 0.001);
	...
	while (calcTrajectories(start_node) && ros::Time::now() < plan_end_time - ros::Duration(20 * 0.001) ) { //&& trajToBins.size() < 5) {
		...
		start_node = PointNode(prev_node.point, prev_node.time + ros::Duration(DROP_TIME * 0.001));
	}

	std::cout << "Proccessing took " << ros::Time::now() - plan_start_time << "ms (" << PLAN_TIME_OFFSET << " allowed)." << std::endl;
	...
	while (index < trajToBins.size()) {
		...
		// Lower arm
		arm.lowerArm();
		ros::Duration(DROP_TIME * 0.001).sleep();

		// Get to bin
		arm.followTrajectory(trajToBins[index]);
		ros::Duration(MOTOR_DELAY * 0.001).sleep();
...
bool MainPlanner::findCoins() {
	...
	float tableSpeed = 0;
	ros::Time t0;
	bool success = imgProc.run(mode, 40, coins, tableSpeed, t0);
	if (!success) {
		return false;
	}
...
bool MainPlanner::calcTrajectories(PointNode start_node) {
	...
	for (int i = 0; i < items.size(); i++) {
		...
		PointNode next_node(trajToCoin[last_index].point, trajToCoin[last_index].time + ros::Duration(DROP_TIME * 0.001));
	...
	ros::Time least_time = (tempTrajToBins[0])[tempTrajToBins.size() - 1].time;
	int best_index = 0;
	for (int k = 1; k < tempTrajToBins.size(); k++) {
		ros::Time this_time = (tempTrajToBins[k])[tempTrajToBins.size() - 1].time;
...
vector<PointNode> MainPlanner::trajToItem(Item item, PointNode start_node) {
	...
	int time1 = item.getPos(start_node.time).distance(start_node.point) / speed;
	time1 += DROP_TIME;
	ros::Duration dur1((time1 + ADJUST_FACTOR) * 0.001);
	ros::Duration dur2(time1);

	PointNode target(item.getPos(start_node.time + dur1), start_node.time + dur2);
