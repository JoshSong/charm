...
bool ImageProcessor::run(int mode, int numFrames, std::vector<Object> &output_coins,
		float &tableSpeed, ros::Time& lastCamTime)
{
	...
	while (frame_count < numFrames)
	{
		...
		pthread_mutex_lock(&camMutex);
		ros::Time camTime = imageTimestamp;
		...
		if (FindFrameAndCoins(img, cameraMatrix, distCoeffs, img_output, contours, possible_coins, table_rotation, evp))
		{
			...
			if (frame_count > 1)
			{
				ros::Duration dur = camTime - lastCamTime;
				float dt = dur.toSec();
...
bool FindFrameAndCoins(cv::Mat &image, cv::Mat &cameraMatrix, cv::Mat &distCoeffs, cv::Mat &output_image, vector<vector<cv::Point> > &contours, vector<Coin> &output, float &table_rotation, EllipseValidationParameters evp)
{
	...
	if (frame_centre_found)
	{
		...
		for (int l = 0; l < outer_frame_ellipses.size(); l++)
		{
			...
			if (bounds.contains(outer_frame_ellipses[l].center)) {
				...
				frame_marker_mask_ellipse.center -= cv::Point2f(frame_marker_bound.tl().x, frame_marker_bound.tl().y);
	...
	for (int i = 0; i < num_circle_points; i++) {
		transformed_circle_pointsf.push_back(cv::Point2f(transformed_circle_points[i].x, transformed_circle_points[i].y));
	}
...
void ImageProcessor::calibrate()
{
	...
	while (1)
	{
		...
		char buf[256];
		sprintf(buf, "Images remaing: %d", int(numRequiredFrames - imagePoints.size()));
